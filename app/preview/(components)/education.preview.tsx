...
const PEducation = ({
    ...
    return (<Fragment><div className="grid grid-cols-1 gap-2.5 h-max">
        <h3 className="text-xl font-semibold text-pretty">Education:</h3>
        <div className="flex flex-nowrap gap-2.5 flex-col">
            <span className="text-sm font-medium text-pretty">{education.university.title}</span>
            <span className="text-sm text-gray-600">
                {education.university.degree} ({education.university.startDate} - {education.university.endDate})
            </span>
        </div>
    </div>
        ...
        <div className="grid grid-cols-1 gap-2.5 h-max">
            ...
            <div className="flex items-center gap-2.5">
                {languages.map((language, index) => (
                    <span key={`details-languages-${language}-${index}`} className="text-sm text-gray-600">{language}{languages.length - 1 !== index ? ',' : ""}</span>
                ))}
            </div>
        </div>
    </Fragment>)
